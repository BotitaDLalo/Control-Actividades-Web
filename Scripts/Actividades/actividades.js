async function cargarActividadesDeMateria() {
  ...
  try {
    ...
    const response = await fetch(
      `/Materias/ObtenerActividadesPorMateria?materiaId=${materiaId}`
    );
    ...
    if (!response.ok) {
      const mensaje =
        payload && (payload.mensaje || payload.message)
          ? payload.mensaje || payload.message
    ...
    let actividades = payload.Actividades;

    let rolUsuario = payload.RolUsuario;

    renderizarActividades(actividades, rolUsuario);
  } catch (error) {
...
function renderizarActividades(actividades, rolUsuario) {
  const listaActividades = document.getElementById("listaActividadesDeMateria");
  if (!listaActividades) return;
  ...
  actividades.forEach((actividad) => {
    // skip by filter
    const estadoKey =
      actividad.Enviado === true
        ? "publicada"
        : actividad.Enviado === false
        ? "borrador"
    ...
    const descripcionActividadConEnlace = convertirUrlsEnEnlaces(
    ...
    elemento += opcionPorRol;

    actividadItem.innerHTML = elemento;
    ...
    if (btnEliminar != null) {
      btnEliminar.addEventListener("click", () =>
        eliminarActividad(actividad.ActividadId)
      );
    }
    ...
    if (btnEliminar != null) {
      btnEditar.addEventListener("click", () =>
        editarActividad(actividad.ActividadId)
      );
    }

    const btnIrActividad = actividadItem.querySelector(".btn-ir-actividades");
    if (btnIrActividad != null) {
      btnIrActividad.addEventListener("click", () =>
        IrAActividad(actividad.ActividadId)
      );
    }
...
async function registrarActividad() {
  ...
  if (!descripcion) {
    Swal.fire({
      icon: "warning",
      title: "Campos incompletos",
      text: "La actividad debe tener una descripcion",
    });
    return;
  }
...
async function editarActividad(id) {
  try {
    ...
    const resp = await fetch(
      `/Actividades/ObtenerActividadPorId?actividadId=${id}`
    );
    ...
    document.getElementById("fechaHoraLimite").value = toInputDateTimeValue(
      data.FechaLimite || data.FechaCreacion
    );
...
async function actualizarActividad(id) {
  ...
  try {
    ...
    Swal.fire(
      "Error",
      e.message || "No se pudo actualizar la actividad",
      "error"
    );
  }
}
